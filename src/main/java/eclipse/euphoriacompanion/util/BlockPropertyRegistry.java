...
public class BlockPropertyRegistry {
    ...
    public static synchronized void resetInstance() {
        instance = null;
        EuphoriaCompanion.LOGGER.info("Reset block property registry instance");
    }
    ...
    private void loadRegistry() {
        ...
        if (registryFile.exists()) {
            try (FileReader reader = new FileReader(registryFile)) {
                Map<String, Map<String, Set<String>>> loadedRegistry = gson.fromJson(
                    reader,
                    new TypeToken<Map<String, Map<String, Set<String>>>>(){}.getType()
                );
                
                if (loadedRegistry != null) {
                    usedBlockProperties.putAll(loadedRegistry);
                    EuphoriaCompanion.LOGGER.info("Loaded property registry with {} blocks", usedBlockProperties.size());
                }
            } catch (IOException e) {
                EuphoriaCompanion.LOGGER.error("Failed to load property registry: {}", e.getMessage());
            }
        } else {
            EuphoriaCompanion.LOGGER.info("No existing property registry found, creating new one");
        }
    }
    ...
    public void saveRegistry() {
        ...
        try (FileWriter writer = new FileWriter(registryFile)) {
            ...
            EuphoriaCompanion.LOGGER.info("Saved property registry with {} blocks", usedBlockProperties.size());
        } catch (IOException e) {
    ...
    public void clearAll() {
        ...
        EuphoriaCompanion.LOGGER.info("Cleared all property registry data");
    }
    ...
    public void registerUsedProperty(String blockId, String propertyName, String propertyValue) {
        ...
        if (values.add(normalizedValue)) {
            EuphoriaCompanion.LOGGER.debug("Registered used property {}={} for block {} (normalized from {})", 
                propertyName, normalizedValue, blockId, propertyValue);
        }
    }
    ...
    public void registerBlockWithProperties(String blockIdWithProperties) {
        ...
        BlockPropertyExtractor.ParsedBlockIdentifier parsed = 
            BlockPropertyExtractor.parseBlockIdentifier(blockIdWithProperties);
        ...
        for (BlockPropertyExtractor.BlockStateProperty property : parsed.properties) {
            registerUsedProperty(blockId, property.getName(), property.getValue());
        }
    }
    ...
    public void processAllShaderBlocks(Set<String> shaderBlocks) {
        ...
        EuphoriaCompanion.LOGGER.info("Processed {} shader blocks, found {} blocks with properties", 
            shaderBlocks.size(), usedBlockProperties.size());
    }
    ...
    public Map<String, Set<String>> getAllGamePropertyValues(String blockId) {
        ...
        try {
            ...
            for (Property<?> property : block.getStateManager().getProperties()) {
                ...
                Collection<? extends Comparable<?>> possibleValues = 
                    BlockPropertyExtractor.getPropertyValuesViaReflection(property);
            ...
            EuphoriaCompanion.LOGGER.error("Error getting properties for block {}: {}", 
    ...
    /**
     * Gets all block IDs with used properties
     * 
     * @return Set of block identifiers
     */
    public Set<String> getBlocksWithUsedProperties() {
        return new HashSet<>(usedBlockProperties.keySet());
    }
    
    /**
     * Gets used properties for a block
     * 
     * @param blockId The block identifier
     * @return Map of property names to used values
     */
    public Map<String, Set<String>> getUsedProperties(String blockId) {
        // Ensure block ID has namespace
        if (!blockId.contains(":")) {
            blockId = "minecraft:" + blockId;
        }
        
        return usedBlockProperties.getOrDefault(blockId, Collections.emptyMap());
    }
}